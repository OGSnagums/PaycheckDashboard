...
export const DEFAULTS = {
  ...
  investments: [
    { id: "roth", label: "Roth IRA", amount: 0, icon: "📈", active: true, pct: 10, manualOverride: false },
...
export function migrateState(s) {
  ...
  if (Array.isArray(s.investments)) {
    s.investments = s.investments.map((investment) => {
      if (investment.id !== "roth") return investment;
      return {
        ...investment,
        pct: investment.pct || 10,
        manualOverride: Boolean(investment.manualOverride)
      };
    });
  }
  return s;
